...
type
  ...
  TFGridSettings = class(TForm)
    ...
    edFixed: TSpinEdit;
    Label9: TLabel;
    cbGridColor1: TColorBox;
    Label10: TLabel;
    cbGridColor2: TColorBox;
    ...
    procedure cbGridColor1Change(Sender: TObject);
    procedure cbGridColor2Change(Sender: TObject);
...
procedure TFGridSettings.RefreshExample;
begin
  if not Assigned(CurrColumn) then exit;
  mExample.Color := cbColor.Selected;
  mExample.Font := CurrColumn.Font;
end;
...
procedure TFGridSettings.btnFontClick(Sender: TObject);
begin
  if not Assigned(CurrColumn) then exit;
  FontDialog.Font := CurrColumn.Font;
  if FontDialog.Execute then
  begin
    CurrColumn.Font := FontDialog.Font;
    RefreshExample;
  end;
end;

procedure TFGridSettings.btnGridFontClick(Sender: TObject);
begin
  FontDialog.Font := FDBGrid.Font;
  if FontDialog.Execute then FDBGrid.Font := FontDialog.Font;
end;

procedure TFGridSettings.btnResetClick(Sender: TObject);
begin
  if Application.MessageBox(pchar('Сбросить настройки таблицы? ' +
    'В результате этой операции будут установлены стандартные настройки.'), 'Подтверждение',
...
procedure TFGridSettings.cbAlignChange(Sender: TObject);
begin
  if not Assigned(CurrColumn) then exit;
  CurrColumn.Alignment := TAlignment(cbAlign.ItemIndex);
end;

procedure TFGridSettings.cbColorChange(Sender: TObject);
begin
  if not Assigned(CurrColumn) then exit;
  CurrColumn.Color := cbColor.Selected;
  RefreshExample;
end;

procedure TFGridSettings.cbGridColor1Change(Sender: TObject);
var
  i: integer;
  col: TColumnEh;

begin
  FDBGrid.Color := cbGridColor1.Selected;
  for i := 0 to FDBGrid.Columns.Count - 1 do
  begin
    col := FDBGrid.Columns[i];
    col.Color := FDBGrid.Color;
  end;
end;

procedure TFGridSettings.cbGridColor2Change(Sender: TObject);
begin
  FDBGrid.EvenRowColor := cbGridColor2.Selected;
end;
...
begin
  ...
  cbAlign.Enabled := AEnabled;
  Label6.Enabled := AEnabled;
  edWidth.Enabled := AEnabled;
  Label7.Enabled := AEnabled;
  cbColor.Enabled := AEnabled;
  Label8.Enabled := AEnabled;
  btnFont.Enabled := AEnabled;
end;
...
begin
  ...
  if CanCreateColumns then
    ...
    begin
      ...
      colTo.Alignment := colFrom.Alignment;
      colTo.Width := colFrom.Width;
      colTo.Color := colFrom.Color;
      colTo.Font.Name := colFrom.Font.Name;
      colTo.Font.Color := colFrom.Font.Color;
      colTo.Font.Size := colFrom.Font.Size;
      colTo.Font.Style := colFrom.Font.Style;
    end;
    
  // копируем свойства грида
  GridTo.Color := GridFrom.Color;
  GridTo.EvenRowColor := GridFrom.EvenRowColor;
  GridTo.Font.Name := GridFrom.Font.Name;
  GridTo.Font.Color := GridFrom.Font.Color;
  GridTo.Font.Size := GridFrom.Font.Size;
  GridTo.Font.Style := GridFrom.Font.Style;
...
begin
  ...
  cbAlign.ItemIndex := Ord(Col.Alignment);
  edWidth.Value := Col.Width;
  cbColor.Selected := Col.Color;
  CurrColumn := Col;
  RefreshExample;
end;
...
begin
  ...
  try
    ...
    cbGridColor1.Selected := FDBGrid.Color;
    cbGridColor2.Selected := FDBGrid.EvenRowColor;
