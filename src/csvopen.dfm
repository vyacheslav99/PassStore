object FOpenCSV: TFOpenCSV
  Left = 0
  Top = 0
  Caption = 'FOpenCSV'
  ClientHeight = 159
  ClientWidth = 342
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  DesignSize = (
    342
    159)
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 10
    Top = 11
    Width = 26
    Height = 13
    Caption = #1060#1072#1081#1083
  end
  object edFile: TJvFilenameEdit
    Left = 47
    Top = 8
    Width = 285
    Height = 21
    DefaultExt = 'txt'
    Filter = 'Text (*.txt)|*.txt|CSV (*.csv)|*.csv'
    DialogTitle = #1042#1099#1073#1088#1072#1090#1100
    Anchors = [akLeft, akTop, akRight]
    TabOrder = 0
  end
  object GroupBox1: TGroupBox
    Left = 10
    Top = 35
    Width = 322
    Height = 78
    Anchors = [akLeft, akTop, akRight]
    Caption = ' '#1055#1072#1088#1072#1084#1077#1090#1088#1099' CSV '
    TabOrder = 1
    object Label2: TLabel
      Left = 14
      Top = 48
      Width = 66
      Height = 13
      Caption = #1056#1072#1079#1076#1077#1083#1080#1090#1077#1083#1100
    end
    object chbQuote: TCheckBox
      Left = 14
      Top = 21
      Width = 171
      Height = 17
      Caption = #1054#1073#1088#1072#1084#1083#1103#1090#1100' '#1089#1090#1088#1086#1082#1080' '#1074' '#1082#1072#1074#1099#1095#1082#1080
      TabOrder = 0
    end
    object chbIncColumns: TCheckBox
      Left = 146
      Top = 47
      Width = 167
      Height = 17
      Caption = #1042#1082#1083#1102#1095#1080#1090#1100' '#1085#1072#1079#1074#1072#1085#1080#1103' '#1082#1086#1083#1086#1085#1086#1082
      TabOrder = 2
    end
    object cbSep: TComboBox
      Left = 86
      Top = 45
      Width = 50
      Height = 21
      ItemHeight = 13
      TabOrder = 1
    end
  end
  object btnOk: TBitBtn
    Left = 148
    Top = 124
    Width = 90
    Height = 27
    Anchors = [akRight, akBottom]
    Caption = #1042#1099#1087#1086#1083#1085#1080#1090#1100
    Default = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ModalResult = 1
    ParentFont = False
    TabOrder = 2
    Glyph.Data = {
      
      FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
  end
  object btnCancel: TBitBtn
    Left = 244
    Top = 124
    Width = 90
    Height = 27
    Anchors = [akRight, akBottom]
    Cancel = True
    Caption = #1054#1090#1084#1077#1085#1072
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ModalResult = 2
    ParentFont = False
    TabOrder = 3
    Glyph.Data = {
      
      FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
  end
end
