...
begin
  if (index < 0) or (index >= Length(FLabels)) then
    raise Exception.Create('Индекс метки вышел за границы массива!');
...
begin
  ...
  begin
    ...
    try
      ...
      on e: Exception do
        Application.MessageBox(pchar('Ошибка в строке фильтра! Задано неверное выражение для одного из полей! ' +
          'Одно из выражений не соответствует типу поля.'#13 + e.Message), 'Ошибка', MB_OK + MB_ICONERROR);
    end;
  end;
  Action := caHide;
end;
...
begin
  ...
  try
    ...
    on e: Exception do
      Application.MessageBox(pchar('Ошибка при определении текущего фильтра! Возможность добавить ' +
        'новый фильтр к текущему будет недоступна. Сообщение ошибки:'#13 + e.Message),
